...
@description('Base name used to derive default resource names (e.g. "ci" -> "ci-app", "ci-app-env"). Keep stable across runs to make CD idempotent.')
param baseName string = 'ci'

@description('Resource group name. Defaults to "<baseName>-app-rg". Override via RESOURCE_GROUP_NAME env var.')
param resourceGroupName string = ''

@description('Container App name (the FQDN segment before the ACA env domain). Defaults to "<baseName>-app".')
param applicationName string = ''

@description('Container Apps managed environment name. Its default domain is generated by Azure and is preserved across runs as long as the env is reused.')
param containerAppsEnvName string = ''

@description('Container Registry name (alphanumeric only). Defaults to a deterministic value derived from baseName.')
param containerRegistryName string = ''

@description('User-assigned managed identity name. Defaults to "<baseName>-app-identity".')
param managedIdentityName string = ''

@description('Log Analytics workspace name. Defaults to "<baseName>-app-logs".')
...
// All resources use stable, parameterized names so CD is idempotent:
// first run creates them, subsequent runs upsert in place. The ACA env's
// default FQDN domain (e.g. delightfulwater-573839ce...) is generated by
// Azure on creation and is preserved as long as the env is reused.
var resolvedResourceGroupName = empty(resourceGroupName) ? '${baseName}-app-rg' : resourceGroupName
var resolvedApplicationName = empty(applicationName) ? '${baseName}-app' : applicationName
var resolvedContainerAppsEnvName = empty(containerAppsEnvName) ? '${baseName}-app-env' : containerAppsEnvName
var resolvedContainerRegistryName = empty(containerRegistryName) ? replace('${baseName}appacr', '-', '') : containerRegistryName
var resolvedManagedIdentityName = empty(managedIdentityName) ? '${baseName}-app-identity' : managedIdentityName
var resolvedLogAnalyticsName = empty(logAnalyticsName) ? '${baseName}-app-logs' : logAnalyticsName
