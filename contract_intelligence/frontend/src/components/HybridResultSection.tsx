  ...
  useEffect(() => {
    const renderDiagram = async (diagramCode: string) => {
      ...
      try {
        ...
        // Show loading state while attempting to fix
        if (ref.current && fixAttempts < maxAttempts) {
          ref.current.innerHTML = `
            <div class="p-4 bg-blue-500/10 border border-blue-400/30 rounded-lg flex items-center gap-3">
              <svg class="animate-spin h-5 w-5 text-blue-400" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24">
                <circle class="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" stroke-width="4"></circle>
                <path class="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4zm2 5.291A7.962 7.962 0 014 12H0c0 3.042 1.135 5.824 3 7.938l3-2.647z"></path>
              </svg>
              <p class="text-blue-400 text-sm">Auto-correcting diagram (attempt ${fixAttempts + 1}/${maxAttempts})...</p>
            </div>
          `;
        }
        ...
        if (fixed && fixed !== diagramCode) {
          ...
          // Show error only if fix failed or max attempts reached
          setHasError(true);
          ref.current.innerHTML = `
